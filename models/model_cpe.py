...
class CLIPParameterEfficient(nn.Module):
    ...
    def forward(
        ...
        if text_tokens.shape[1]+L_g>max_length:
            text_tokens_temp = text_tokens[:,0:max_length-L_g]
        else:
            text_tokens_temp = text_tokens

        if attn_mask.shape[1]+L_g>max_length:
            attn_mask_temp = attn_mask[:,0:max_length-L_g]
        else:
            attn_mask_temp = attn_mask
        ...
        text_projs = self.text_proj(text_out)
        img_projs = self.image_proj(img_out) 
        # Normalize
        text_embed = text_projs / _get_vector_norm(text_projs)
        img_embed = img_projs / _get_vector_norm(img_projs)

        # cosine similarity as logits
        logit_scale = self.clip_model.logit_scale.exp()
        logits_per_text = torch.matmul(text_embed, img_embed.t().to(text_embed.device)) * logit_scale.to(text_embed.device)
...
class FSCIL_classrroom_CPE(nn.Module):
    def __init__(self, cfg, device, jit=False):
        super().__init__()
        self.prompt_template = cfg.prompt_template
        self.device = device
        self.classes_names = None
        self.model = CLIPParameterEfficient(L_g=cfg.L_g, deep_g=cfg.deep_g, 
    ...
    def adaptation(self, task_id, cfg, train_dataset, train_classes_names):
        ...
        out_text_tokens = self.text_preprocess([self.prompt_template.format(c) for c in self.current_class_names], padding=True, return_tensors="pt")
