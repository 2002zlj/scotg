...
def set_up_datasets(args):
    ...
    if args.dataset == 'mini_imagenet':
        ...
        args.shot = 5
        args.sessions = 9
    if args.dataset == 'classroom':
        import dataloader.classroom.classroom as Dataset
        args.base_class = 20
        args.num_classes=32
        args.way = 3
        args.shot = 5
        args.sessions = 5
    if args.dataset == 'classroom_1shot':
        import dataloader.classroom.classroom as Dataset
        args.base_class = 20
        args.num_classes=32
        args.way = 3
        args.shot = 1
        args.sessions = 5
    if args.dataset == 'classroom_3shot':
        import dataloader.classroom.classroom as Dataset
        args.base_class = 20
        args.num_classes=32
        args.way = 3
...
def get_base_dataloader(args, clip_trsf=None):
    txt_path = "/data_25T/zlj/FSCIL/PriViLege-main_me/data/index_list/" + args.dataset + "/session_" + str(0 + 1) + '.txt'
    class_index = np.arange(args.base_class)
    if args.dataset == 'cifar100':

        if args.pret_clip:
            trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True, transform=clip_trsf,
                                            index=class_index, base_sess=True,is_vit=args.vit, is_clip=args.pret_clip)
            testset = args.Dataset.CIFAR100(root=args.dataroot, train=False, download=False,
                                            index=class_index, base_sess=True,is_vit=args.vit, is_clip=args.pret_clip)
        elif args.clip_text:
            trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True, transform=clip_trsf,
                                         index=class_index, base_sess=True,is_vit=args.vit, is_clip=args.clip_text)
            testset = args.Dataset.CIFAR100(root=args.dataroot, train=False, download=False,
                                            index=class_index, base_sess=True,is_vit=args.vit, is_clip=args.clip_text)
        elif args.vit:
            trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True, transform=clip_trsf,
    ...
    if args.dataset == 'cub200':
        trainset = args.Dataset.CUB200(root=args.dataroot, train=True,
                                       index=class_index, base_sess=True, is_clip=args.pret_clip)
        testset = args.Dataset.CUB200(root=args.dataroot, train=False, index=class_index, is_clip=args.pret_clip)

    if args.dataset == 'mini_imagenet':
        trainset = args.Dataset.MiniImageNet(root=args.dataroot, train=True,
                                             index=class_index, base_sess=True, is_clip=args.pret_clip)
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False, index=class_index, is_clip=args.pret_clip)

    if args.dataset == 'classroom' or args.dataset == 'classroom_1shot' or args.dataset == 'classroom_3shot':
        trainset = args.Dataset.Classroom_dataset(root=args.dataroot, train=True,
                                             index=class_index, base_sess=True, is_clip=args.pret_clip)
        testset = args.Dataset.Classroom_dataset(root=args.dataroot, train=False, index=class_index, is_clip=args.pret_clip)

    trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_size=args.batch_size_base, shuffle=True,
                                              num_workers=0)
    testloader = torch.utils.data.DataLoader(
        dataset=testset, batch_size=args.test_batch_size, shuffle=False, num_workers=0)
...
def get_base_dataloader_meta(args):
    ...
    if args.dataset == 'cub200':
        trainset = args.Dataset.CUB200(root=args.dataroot, train=True,
                                       index_path=txt_path, is_clip=args.pret_clip)
        testset = args.Dataset.CUB200(root=args.dataroot, train=False,
                                      index=class_index, is_clip=args.pret_clip)
    if args.dataset == 'mini_imagenet':
        trainset = args.Dataset.MiniImageNet(root=args.dataroot, train=True,
                                             index_path=txt_path, is_clip=args.pret_clip)
...
def get_new_dataloader(args,session, clip_trsf=None):
    txt_path = "/data_25T/zlj/FSCIL/PriViLege-main/data/index_list/" + args.dataset + "/session_" + str(session + 1) + '.txt'
    if args.dataset == 'cifar100':
        if args.pret_clip:
            class_index = open(txt_path).read().splitlines()
            trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=False, transform=clip_trsf,
                                            index=class_index, base_sess=False,is_vit=args.vit, is_clip=args.pret_clip)
        elif args.clip_text:
            class_index = open(txt_path).read().splitlines()
            trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=False, transform=clip_trsf,
                                            index=class_index, base_sess=False,is_vit=args.vit, is_clip=args.clip_text)            
        else:
            class_index = open(txt_path).read().splitlines()
    ...
    if args.dataset == 'mini_imagenet':
        trainset = args.Dataset.MiniImageNet(root=args.dataroot, train=True,
                                       index_path=txt_path, is_clip=args.pret_clip)
    if args.dataset == 'classroom' or args.dataset == 'classroom_1shot' or args.dataset == 'classroom_3shot':
        index = np.array([x+20+3*(session-1) for x in range(3)])
        trainset = args.Dataset.Classroom_dataset(root=args.dataroot, train=True,
                                       index=index, is_clip=args.pret_clip)
    ...
    if args.dataset == 'cifar100':
        if args.pret_clip:
            testset = args.Dataset.CIFAR100(root=args.dataroot, train=False, download=False,
                                            index=class_new, base_sess=False,is_vit=args.vit, is_clip=args.pret_clip)
        elif args.clip_text:
            testset = args.Dataset.CIFAR100(root=args.dataroot, train=False, download=False,
                                            index=class_new, base_sess=False,is_vit=args.vit, is_clip=args.clip_text)
        elif args.vit:
            testset = args.Dataset.CIFAR100(root=args.dataroot, train=False, download=False, transform=clip_trsf,
                                            index=class_new, base_sess=False,is_vit=args.vit, is_clip=args.pret_clip)
    if args.dataset == 'cub200':
        testset = args.Dataset.CUB200(root=args.dataroot, train=False,
                                      index=class_new, is_clip=args.pret_clip)
    if args.dataset == 'mini_imagenet':
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False,
                                      index=class_new, is_clip=args.pret_clip)
    if args.dataset == 'classroom' or args.dataset == 'classroom_1shot' or args.dataset == 'classroom_3shot':
        testset = args.Dataset.Classroom_dataset(root=args.dataroot, train=False,
                                      index=class_new, is_clip=args.pret_clip)
