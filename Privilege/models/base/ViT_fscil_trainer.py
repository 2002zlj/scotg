...
class ViT_FSCILTrainer(Trainer):
    def __init__(self, args):
        ...
        if self.args.pret_clip:
            self.val_model = CLIP_ViT_MYNET(self.args, mode=self.args.base_mode)
        else:
        ...
        self.save_path = '/data_25T/zlj/FSCIL/PriViLege-main_me/result/Privilege/classroom/'+str(self.args.shot)+'_shot/'+self.current+ "/"+ time.strftime("%H_%M", time.localtime())
        os.makedirs(self.save_path,exist_ok=True)
        #* Bert_model for ViT-B
        # self.tokenizer = BertTokenizer.from_pretrained('bert-base-cased')
        # self.Bert_model = BertModel.from_pretrained("bert-base-cased")
        self.tokenizer = BertTokenizer.from_pretrained('/data_25T/zlj/FSCIL/PriViLege-main/ckp/bert-base-cased')
        self.Bert_model = BertModel.from_pretrained("/data_25T/zlj/FSCIL/PriViLege-main/ckp/bert-base-cased")
    ...
    def train(self):
            ...
            if session == 0:  # load base class train img label
                ...
                for epoch in range(args.epochs_base):
                    ...
                    if (tsa * 100) >= self.trlog['max_acc'][session]:
                        ...
                        save_model_dir = os.path.join(self.save_path, 'session' + str(session) + '_max_acc.pth')
                ...
                save_model_dir = os.path.join(self.save_path, 'session' + str(session) + '_last_epoch.pth')
                torch.save(dict(params=self.model.state_dict()), save_model_dir)
                ...
                if not args.not_data_init:
                    ...
                    if not args.pret_clip:
                        ...
                        best_model_dir = os.path.join(self.save_path, 'session' + str(session) + '_max_acc_replace_head.pth')
        ...
        save_list_to_txt(os.path.join(self.save_path, 'results.txt'), result_list)
    ...
    def test(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                if self.args.shot ==5:
                    ckp_path = '/data_25T/zlj/FSCIL/PriViLege-main_me/weight/5_shot/session0_max_acc_replace_head.pth'
                elif self.args.shot ==3:
                    ckp_path = '/data_25T/zlj/FSCIL/PriViLege-main_me/weight/3_shot/session0_max_acc_replace_head.pth'
                elif self.args.shot ==1:
                    ckp_path = '/data_25T/zlj/FSCIL/PriViLege-main_me/weight/1_shot/session0_max_acc_replace_head.pth'

                ckp = torch.load(ckp_path)
                self.model.load_state_dict(ckp['params'])
                tsl, tsa, logs = test(self.model, testloader, 0, args, session,self.word_info)
                self.trlog['max_acc'][session] = float('%.2f' % (tsa * 100))
                ...
                trainloader.dataset.transform = testloader.dataset.transform
                if self.args.shot ==5:
                    ckp_path = '/data_25T/zlj/FSCIL/PriViLege-main_me/weight/5_shot/session'+str(session)+'_max_acc.pth'
                elif self.args.shot ==3:  
                    ckp_path = '/data_25T/zlj/FSCIL/PriViLege-main_me/weight/3_shot/session'+str(session)+'_max_acc.pth'
                elif self.args.shot ==1:
                    ckp_path = '/data_25T/zlj/FSCIL/PriViLege-main_me/weight/1_shot/session'+str(session)+'_max_acc.pth'
                ckp = torch.load(ckp_path)
                self.model.load_state_dict(ckp['params'])
        ...
        result_list = self.trlog['max_acc']
        print(self.trlog['max_acc'])
        with open(os.path.join(self.save_path, 'test_results.txt'), 'w') as f:
            for i,item in enumerate(result_list):
                f.write(f"Acc_{i}: {item}\n")
            f.write(f"Acc_avg: {round(sum(result_list) / len(result_list), 2)}\n")
            f.write(f"Acc_last: {result_list[len(result_list)-1]}\n")
    ...
    def set_save_path(self):
        ...
        self.args.save_path = os.path.join(f'/data_25T/zlj/FSCIL/PriViLege-main_me/checkpoint/{self.args.out}', self.args.save_path)
...
class CLIP_ViT_FSCILTrainer(Trainer):
    def __init__(self, args):
        ...
        self.processor = CLIPProcessor.from_pretrained('/data_25T/zlj/FSCIL/PriViLege-main/ckp/clip-vit-base-patch16')
    ...
    def get_optimizer_base(self):
        ...
        if self.args.schedule == 'Step':
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.args.epochs_base)
        elif self.args.schedule == 'Cosine_warmup':
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                print("Build Word Information..")
                if args.pret_clip:
                    build_label_embedding_CLIP(train_set, session, self.model.module.CLIP_model, self.processor, self.word_info, args)
                else:
...
class CLIP_ViT_FSCILTrainer_classroom(Trainer):
    def __init__(self, args):
        super().__init__(args)
        self.current = time.strftime("%Y_%m_%d", time.localtime())
        self.args = args
        ...
        self.args = set_up_datasets(self.args)
        self.model = CLIP_ViT_MYNET_classroom(self.args, mode=self.args.base_mode)
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                print("Build Word Information..")
                if args.pret_clip:
                    build_label_embedding_classroom(train_set, session, self.Bert_model, self.tokenizer, self.word_info, args)
                else:
                    print('Wrong! You should choose pret_clip')
                print("Total Word vector info:", self.word_info["embed"].shape)
                print("Current Word vector info:", self.word_info["cur_embed"].shape)
                ...
                for epoch in range(args.epochs_base):
                    start_time = time.time()
                    # train base sess
                    tl, ta = base_train_classroom(self.model, trainloader, optimizer, scheduler, epoch, self.word_info, self.query_info, np.unique(train_set.targets), args, self.loss_curve)
                    tsl, tsa, logs = test_classroom(self.model, testloader, epoch, args, session,self.word_info)
                    if (tsa * 100) >= self.trlog['max_acc'][session]:
                        self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
                        self.trlog['max_acc_epoch'] = epoch
                        save_model_dir = os.path.join(args.save_path, 'session' + str(session) + '_max_acc.pth')
