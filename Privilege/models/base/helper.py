...
def build_label_embedding_CLIP(train_set,session,CLIP_model,tokenizer,word_info, args):
    if args.dataset == "cifar100":
        ...
        with open('/data_25T/zlj/FSCIL/PriViLege-main_me/data/class_discription/'+'cifar/description.json','r') as f:
            class_discription_dict = json.load(f)
    ...
    with torch.no_grad():
        CLIP_model.eval()
        if args.dataset in ['cifar100', 'mini_imagenet','classroom']:
            for cls in classes[classes_int]:
                if args.pret_clip or args.clip_text:
                    if args.text_type =='all':
                        text = class_discription_dict[cls]['all_info']
                    elif args.text_type =='only_sentence':
                        text = class_discription_dict[cls]['only_sentence_info']
                    elif args.text_type =='only_ER':
                        text = class_discription_dict[cls]['only_EA_info']
                    else:
                        text = f'a photo of {cls}'
                    # 分块
                    max_len = CLIP_model.config.text_config.max_position_embeddings
                    tokens = tokenizer.tokenize(text)
                    tokens = [x[:-4] for x in tokens]
                    if len(tokens)>max_len:
                        chunks = [tokens[i:i + max_len] for i in range(0, len(tokens), max_len)]               
                        encodings = []         
                        for chunk in chunks:
                            encoded_input = tokenizer(" ".join(chunk), return_tensors="pt", truncation=True, max_length=max_len)
                            encoded_input = {key: value.cuda() for key, value in encoded_input.items()}
                            outputs = CLIP_model.get_text_features(**encoded_input)  # 仅对文本特征编码
                            encodings.append(outputs)
                            # 聚合编码（平均）
                        output = torch.mean(torch.stack(encodings), dim=0)
                    else:
                        encoded_input = tokenizer(text=" ".join(tokens),return_tensors="pt",padding=True, truncation=True, max_length=max_len)
                        # 将输入张量移动到 CUDA 设备
                        encoded_input = {key: value.cuda() for key, value in encoded_input.items()}
                        output = CLIP_model.get_text_features(**encoded_input)    # 512维
...
def build_label_embedding(train_set,session,Bert_model,tokenizer,word_info, args):
    if args.dataset == "cifar100":
        ...
        print("classes_int:",classes_int)
        print('new classes for session {} : {} \n'.format(session, classes))
    elif args.dataset == "classroom"  or args.dataset == 'classroom_1shot' or args.dataset == 'classroom_3shot':
        classes = train_set.class_names
        print("Number of classes:", len(classes))
        classes_int = np.unique(train_set.targets)
        print("classes_int:",classes_int)
    ...
    with torch.no_grad():
        Bert_model.eval()
        if args.dataset in ['cifar100', 'mini_imagenet','classroom','classroom_1shot','classroom_3shot']:
            for cls in classes[classes_int]:
                if args.pret_clip:
...
def build_label_embedding_classroom(train_set,session,Bert_model,tokenizer,word_info, args):
    if args.dataset == "cifar100":
        classes = np.unique(train_set.classes)
        print("Number of classes:", len(classes))
        classes_int = np.unique(train_set.targets)
...
def replace_base_fc(trainset, transform, model, args):
    ...
    model.module.fc.weight.data[:args.base_class] = proto_list                           # 用原型分类器替换了分类器

    return model


def replace_base_fc_CLIP(trainset, transform, model, args):
    print("[Replace Base FC - Original]")
    model = model.eval()
...
def base_train(model, trainloader, optimizer, scheduler, epoch, word_info, query_info, class_list, args, loss_curve):
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        if args.ED:
            loss_tri = triplet(cls_embed, prompt_embed['Vision'], query_info, train_label)
        else:
            loss_tri = torch.zeros(1,device='cuda')
        
        if args.SKD:
            loss_kb = knowledge_boosting(prompt_embed['Language'], word_embed, query_info, train_label)
...
def base_train_classroom(model, trainloader, optimizer, scheduler, epoch, word_info, query_info, class_list, args, loss_curve):
    print("[Base Train]")
    base_mode = model.module.mode
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        if args.SKD:
            loss_kb = knowledge_boosting(prompt_embed['Language_noproject'], word_embed, query_info, train_label)
            # loss_kb = knowledge_boosting(prompt_embed['Language'], word_embed, word_cur_embed, train_label)
        else:
            loss_kb = torch.zeros(1,device='cuda')
        ...
        grad['expert_prompt']=model.module.expert_prompt.grad.clone().detach().cpu()
        grad['prompt']=model.module.prompt.grad.clone().detach().cpu()
        for n, p in model.module.CLIP_model.vision_model.encoder.layers[:2].named_parameters():
            if 'self.attn.weight' in n:
                grad[n] = torch.norm(p.clone().detach().cpu(),p=2,dim=1).mean()
        loss_curve['grad_list'].append(grad)
...
def base_train_CLIP(model, trainloader, optimizer, scheduler, epoch, word_info, query_info, class_list, args, loss_curve):
    print("[Base Train]")
    base_mode = model.module.mode
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        if args.SKD:
            loss_kb = knowledge_boosting_CLIP(prompt_embed['Language_project'], prompt_embed['Language_noproject'],word_embed, query_info, train_label)
            # loss_kb = knowledge_boosting(prompt_embed['Language'], word_embed, word_cur_embed, train_label)
        else:
            loss_kb = torch.zeros(1,device='cuda')
...
def base_train_CLIP_text(model, trainloader, optimizer, scheduler, epoch, word_info, query_info, class_list, args, loss_curve):
    print("[Base Train]")
    base_mode = model.module.mode
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        data, train_label = [_.cuda() for _ in batch]
        cls_embed, prompt_embed = model(data, train_label, base=True)
        ...
        logits_text = get_text_logits(image_embeds=text_image_embeds,text_embeds=word_embed,model=model)
        logits_pro = get_pro_logits(image_embeds=pro_image_embeds,pro_embeds=pro_embed,model=model)
        logits_ = gamma*logits_text+(1-gamma)*logits_pro
        loss_ce = F.cross_entropy(logits_, train_label)
        
        if args.ED:
            loss_tri = triplet_text(cls_embed, prompt_embed['Language'], word_embed, train_label)
        else:
            loss_tri = torch.zeros(1,device='cuda')
        
        if args.SKD:
            loss_kb = knowledge_boosting_CLIP_text(prompt_embed['Language'],word_embed, query_info, train_label)
            # loss_kb = knowledge_boosting(prompt_embed['Language'], word_embed, word_cur_embed, train_label)
        else:
            loss_kb = torch.zeros(1,device='cuda')
...
def get_text_logits(image_embeds,text_embeds,model):
    # normalized features
...
def get_pro_logits(image_embeds,pro_embeds,model):
    # normalized features
    image_embeds = image_embeds / image_embeds.norm(p=2, dim=-1, keepdim=True)
...
def triplet(cls_embed, vision_embed, query_info, train_label):
...
def triplet_text(cls_embed, language_embed, word_embed, train_label):
    P_head = word_embed
    
    cls_logit = F.linear(cls_embed, P_head)
    cls_gt = F.cross_entropy(cls_logit, train_label, reduction='none')   #* B
    vis_logit = F.linear(language_embed, P_head)
    vis_gt = F.cross_entropy(vis_logit, train_label, reduction='none')   #* B
...
def knowledge_boosting(lang_embed, word_embed, query_info, train_label):
    T = 2.
...
def knowledge_boosting_CLIP(lang_embed_project, lang_embed_noproject,word_embed, query_info, train_label):
    T = 2.
    idx= torch.arange(len(train_label))
    #* Original
    P_head = query_info['proto'].clone().cuda()
    ...
    #* =======================================================================
    lang_logit = F.linear(lang_embed_noproject, P_head)    #* Soft pred
    loss_seman = F.cross_entropy(lang_logit, train_label)
    #* KL Feature
    loss_kd = F.kl_div(F.log_softmax(lang_embed_project/T,dim=1), F.softmax(word_embed[train_label]/T,dim=1), reduction='batchmean')
    
    loss = loss_kd + 0.2*loss_seman
    return 0.1*loss


def knowledge_boosting_CLIP_text(lang_embed, word_embed, query_info, train_label):
    T = 2.
    idx= torch.arange(len(train_label))
    #* Original
    P_head = word_embed
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            if args.pret_clip:
                ...
                logits = model(data, test_label,B_tuning=True)
...
def test_classroom(model, testloader, epoch, args, session, word_info):
    #todo Test시 Prompt Selection is needed..
    test_class = args.base_class + session * args.way
    model = model.eval()
    vl = Averager_Loss()
...
def test_CLIP_text(model, testloader, epoch, args, session, word_info,query_info,base=False):
    #todo Test시 Prompt Selection is needed..
    test_class = args.base_class + session * args.way
    model = model.eval()
    vl = Averager_Loss()
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            data, test_label = [_.cuda() for _ in batch]
            cls_embed, prompt_embed = model(data, test_label, base=True)
            text_image_embeds = alpha*cls_embed+(1-alpha)*prompt_embed['Language']
            pro_image_embeds = bata*cls_embed+(1-bata)*prompt_embed['Vision']            
            logits_text = get_text_logits(image_embeds=text_image_embeds,text_embeds=word_embed,model=model)
            logits_pro = get_pro_logits(image_embeds=pro_image_embeds,pro_embeds=pro_embed,model=model)
            logits = gamma*logits_text+(1-gamma)*logits_pro
            loss_ce = F.cross_entropy(logits, test_label)
            if args.ED:
                loss_tri = triplet_text(cls_embed, prompt_embed['Language'], word_embed, test_label)
            else:
                loss_tri = torch.zeros(1,device='cuda')
            
            if args.SKD:
                loss_kb = knowledge_boosting_CLIP_text(prompt_embed['Language'],word_embed, query_info, test_label)
                # loss_kb = knowledge_boosting(prompt_embed['Language'], word_embed, word_cur_embed, train_label)
            else:
                loss_kb = torch.zeros(1,device='cuda')

            if base:
                loss = loss_ce + args.ED_hp*loss_tri + loss_kb
            else:
...
from tqdm import tqdm
def build_base_proto_CLIP(train_loader, model, query_info, args):
    model = model.eval()
...
def build_base_proto_CLIP_text(train_loader, model, query_info, args):
    model = model.eval()
...
def build_new_proto_CLIP_text(train_loader, model, query_info, args):
    model = model.eval()
    ...
    for class_index in set(train_loader.dataset.targets):
        data_index = (label_list == class_index).nonzero()
        embedding_this = embedding_list[data_index.squeeze(-1)]
        embedding_this = embedding_this.mean(0)
        proto_list.append(embedding_this)
    ...
    proto_list = torch.stack(proto_list, dim=0) #* num_base, feat_dim
    query_info["proto"] = torch.cat([query_info["proto"],proto_list],dim=0)
    model.module.mode = args.base_mode
    model = model.train()
