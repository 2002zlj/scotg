...
class ViT_MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if self.args.dataset in ['cifar100'] or self.args.dataset in ['classroom','classroom_1shot','classroom_3shot'] :
    ...
    def _extract_attn_mlp_feat(self, block, latent_feat):
        with torch.no_grad():
            ...
            B, N, C = xq.shape
            xq = F.linear(xq, weight[:C,:], bias[:C]).reshape(B,  N, attn.num_heads, C // attn.num_heads).permute(0, 2, 1, 3)   #torch.Size([128, 12, 199, 64])
            _B, _N, _C = xk.shape
            ...
            attention = (xq @ xk.transpose(-2, -1)) * attn.scale   #torch.Size([128, 199, 768])
            attention = attention.softmax(dim=-1)
            attention = attn.attn_drop(attention)

            head_attentions = (attention @ xv).transpose(1, 2)  # torch.Size([128, 199, 12, 64])
            attention = head_attentions.reshape(B, N, C)   #torch.Size([128, 199, 768])
            ...
            mlp_feat =  block.mlp(block.norm2(latent_feat))    # torch.Size([128, 199, 768])
...
class CLIP_ViT_MYNET_classroom(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if self.args.dataset in ['cifar100']:
            ...
            self.num_features = 768
        elif self.args.dataset == 'cub200' or self.args.dataset == 'air':
            self.num_features = 768
        else:
            self.num_features = 768
    ...
    def train_inc(self, dataloader, epochs, session, class_list, word_info, query_info):
        ...
        for idx,batch in enumerate(dataloader):
            ...
            optim = torch.optim.SGD(filter(lambda p: p.requires_grad, self.model.parameters()),
                                          lr=self.args.lr_new,
                                          momentum=0.9,
                                          weight_decay=1e-5)
