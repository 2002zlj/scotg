...
def continual_clip(cfg: DictConfig) -> None:
    ...
    finetune_model = CLIPModel.from_pretrained(cfg.model_name)
    finetune_model.cuda()
    finetune_model.eval()
    ...
    for task_id, _ in enumerate(test_dataset):
        ...
        finetune_model.load_state_dict(ckp)
        finetune_model.cuda()
        model.model.cuda()
        ...
        for inputs,targets,task_ids,names   in tqdm(test_loader):
            ...
            all_output =finetune_model(model.text_tokens0,inputs)
