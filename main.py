...
from models.model import FSCIL_classrroom_SCOTG
import time
...
def continual_clip(cfg: DictConfig) -> None:
    ...
    result_save_path = os.path.dirname(__file__)+'/'+  'result/fscil/classroom/'+cfg.method+'/'+cfg.class_order.split('/')[-1][:-5]+'/'+'SGD/CosineAnnealingWarmupRestarts/'+f'batch_{str(cfg.batch_size)}_lr_{str(cfg.lr)}_avgfreq_{str(cfg.avg_freq)}_l2_{str(cfg.l2)}_ls_{str(cfg.ls)}'+'/'+config_name[:-5]+'/'+str(seed)+'/'+f'a1_{str(cfg.a1)}_a2_{str(cfg.a2)}_a3_{str(cfg.a3)}'+'/'+f'first_{str(cfg.first)}_second_{str(cfg.second)}_third_{str(cfg.third)}'+'/'+time_str
    ...
    eval_dataset, classes_names = build_cl_scenarios(
        cfg, is_train=False, mode='val', transforms=val_transforms
    )
    ...
    acc_list_val = []
    acc_list_test = []
    metric_logger_val = Logger(list_subsets=["test"])
    ...
    cfg.log_path = result_save_path+'/log/metrics.json'
    model_save_path = result_save_path+'/ckp'


    log_file_path =result_save_path+'/log/info.log'
    # 1. 获取 logger 实例
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)  # 设置全局日志级别

    # 2. 清除已有的 handlers，防止重复输出（在某些环境下很有用）
    if logger.hasHandlers():
        logger.handlers.clear()

    # 3. 创建一个 FileHandler，用于写入文件
    ...
    model_text_save_path = result_save_path+'/log/model.txt'
    ...
    shutil.copy(os.path.dirname(os.path.abspath(__file__))+'/models/model.py',model_text_save_path)

    for task_id, _ in enumerate(eval_dataset):
        logging.info(f"Evaluation for task {task_id} has started.")
        for name, paras in model.model.named_parameters():
            if paras.requires_grad:
                print(f"trainable:{name}, size:{paras.size()}")
        total_paras = sum(p.numel() for p in model.model.parameters() if p.requires_grad)
        print(f'Total number of trainable parameters:{total_paras}')
        ...
        with open(result_save_path+'/log/metrics_test.json', 'a+') as f:
            f.write(json.dumps({
                ...
                'avg_acc': round(100 * metric_logger_test.average_incremental_accuracy, 2),
                'forgetting': round(100 * metric_logger_test.forgetting, 6),
                'acc_per_task': [round(100 * acc_t, 2) for acc_t in metric_logger_test.accuracy_per_task],
                'bwt': round(100 * metric_logger_test.backward_transfer, 2),
                'fwt': round(100 * metric_logger_test.forward_transfer, 2),
            }) + '\n')
            metric_logger_test.end_task()
            torch.save(model.model.state_dict(), os.path.join(model_save_path, f'{task_id}.pth'))   
...
if __name__ == "__main__":
    seeds = [2022,2,22,222,2222,22222]
    for seed in seeds:
        seed_all(seed)
        continual_clip()   
